...
echo "[step11-fix] Correction fraud_detection.ts + qr_service.ts pour AI + onchain DEMO"
cd "$BACKEND_DIR"
echo "[step11-fix] Dossier backend : $(pwd)"

# 1) Backups de sécurité
for f in fraud_detection.ts services/qr_service.ts; do
  if [ -f "$f" ] && [ ! -f "$f.step11fix.bak" ]; then
    cp "$f" "$f.step11fix.bak"
    echo "[step11-fix] Backup créé : $f.step11fix.bak"
  fi
done
...
echo "[step11-fix] fraud_detection.ts réécrit."

# 3) Réécriture de services/qr_service.ts avec alias encodeContext/decodeContext
...
echo "[step11-fix] services/qr_service.ts réécrit avec alias encodeContext/decodeContext."

# 4) Recompilation + restart PM2
echo "[step11-fix] Compilation TypeScript..."
npx tsc

echo "[step11-fix] Redémarrage PM2..."
pm2 restart "$PM2_NAME" --update-env

echo "[step11-fix] Terminé. Tu peux relancer :"
echo "  ./omniutil_selfcheck.sh"
echo "et retester les endpoints AI (onboard/partner, qr encode/decode, sync/reward)."
